...
| volumeBindingMode のようになりました | 文字列 | 即時、 WaitForFirstConsumer | ボリュームバインドと動的プロビジョニングを実行するタイミングを選択します | すべて | 1.19～1.24 
|===
[TIP]
====
...
== Astra Trident ResourceQuotaオブジェクト
...
=== インストールに失敗した場合は、ResourceQuotaをクリーンアップします
